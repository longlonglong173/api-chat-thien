...
router.post('/', async(req, res) => {
    try {
        ...
        const indexOfBlockedUser = user.blocked_diary.indexOf(reqUserId);
        ...
        if (reqType == 1) {
            if (indexOfBlockedUser == -1) {
                ...
                // console.log('run');
                user.blocked_diary.splice(indexOfBlockedUser, 1);
            ...
            if (indexOfBlockedUser == -1) {
                user.blocked_diary.push(reqUserId);
                user.save();
                res.json({
                    code: 1000,
                    message: 'OK',
